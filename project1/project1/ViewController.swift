...
//        let image = info[UIImagePickerControllerOriginalImage] as! UIImage
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "saveSegue"{
            ...
            if ((nameLabelField.text?.isEmpty) == false && emotionLabel.titleLabel?.text != "Choose an emotion" && imageView.image != UIImage(named: "upload")){
                ...
                if ((descriptionTextField.text?.isEmpty) == false && descriptionTextField.text != "Please enter memory description"){
                    descriptionField=nameLabelField.text!
                    extraDetailField = descriptionTextField.text
                    print("Success 2")
                    imageField = imageView.image!
...
//                    newMemoryItem.image = imageField
                    ...
                    var newItem = myMemo()
                    newItem.emotion = nameField
                    newItem.color = myColor
                    newItem.name = descriptionField
                    newItem.description = extraDetailField
                    newItem.date = dateField
                    newItem.image = imageField
                    newItem1 = newItem
